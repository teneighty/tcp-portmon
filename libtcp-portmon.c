...
void destroy_tcp_port_monitor(tcp_port_monitor_t *p_monitor, void *p_void)
{
	...
	tcp_connection_t *p_current_conn, *p_temp_conn;

	if (NULL != p_monitor || NULL == p_void) {	/* p_void should be NULL in this context */
		return;
	}
	...
	HASH_ITER(hh, p_monitor->hash, p_current_conn, p_temp_conn) {
		HASH_DEL(p_monitor->hash, p_current_conn);
	}
	p_monitor->hash = NULL;
...
void age_tcp_port_monitor(tcp_port_monitor_t *p_monitor, void *p_void)
{
	...
	for (p_node = p_monitor->connection_list.p_head; p_node; ) {
		...
		HASH_DEL(p_monitor->hash, p_conn);
		...
		void *p_void)
{
	...
	if ((p_monitor->port_range_begin <= p_connection->local_port)
		...
		/* HASH_FIND_PTR(p_monitor->hash, p_connection, p_conn_hash); */
		HASH_FIND(hh, p_monitor->hash, p_connection, 
			      sizeof(tcp_connection_node_t), p_conn_hash);
		if (NULL != p_conn_hash ) {
        ...
        HASH_ADD_KEYPTR(hh, p_monitor->hash, &p_node->connection, 
						sizeof(tcp_connection_node_t), &p_node->connection);
...
tcp_port_monitor_t *create_tcp_port_monitor(uint16_t port_range_begin,
		uint16_t port_range_end, tcp_port_monitor_args_t *p_creation_args)
{
	...
	sprintf(p_monitor->key, ":%04X :%04X", port_range_begin, port_range_end);
		...
		int connection_index, char *p_buffer, size_t buffer_size)
{
	...
	if ((item != COUNT) && (connection_index
			> (int) HASH_COUNT(p_monitor->hash) - 1)) {
		return 0;
	}
...
void destroy_tcp_port_monitor_collection(
		tcp_port_monitor_collection_t *p_collection)
{
	tcp_port_monitor_t *p_current_mon, *p_temp_mon;
	tcp_port_monitor_node_t *p_current_node, *p_next_node;
	...
	/* Destroy hash */
	HASH_ITER(hh, p_collection->hash, p_current_mon, p_temp_mon) {
		HASH_DEL(p_collection->hash, p_current_mon);
	}
	p_collection->hash = NULL;
		...
		tcp_port_monitor_t *p_monitor)
{
	...
	HASH_ADD_PTR(p_collection->hash, key, p_monitor);
...
tcp_port_monitor_t *find_tcp_port_monitor(
		...
		uint16_t port_range_begin, uint16_t port_range_end)
{
	tcp_port_monitor_t *p_monitor;
	char key[12];
	...
	/* is monitor in hash? */
	sprintf(key, ":%04X :%04X", port_range_begin, port_range_end);
	HASH_FIND_PTR(p_collection->hash, key, p_monitor);
	return p_monitor;
}
